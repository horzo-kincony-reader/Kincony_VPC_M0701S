...
## Kardynalne zasady pracy (Copilot + solo)

1. **Brak uproszczeń i brak "tymczasowych" obejść:**  
   Przy modyfikacji plików nie stosujemy żadnych uproszczeń ani zakomentowań "na skróty". Zachowujemy w 100% dotychczasową funkcjonalność kodu oraz stron WWW (wszystkie endpointy, zachowania, formaty JSON, MQTT itd.).

2. **Pełne pliki po zaakceptowanej poprawce:**  
   Po każdej zatwierdzonej poprawce generujesz pełny plik `.ino` lub inny modyfikowany plik **w całości** (bez skrótów i pomijania fragmentów), aby uniknąć pomyłek i mieć jednoznaczną wersję referencyjną.

3. **Opis zmian przed kompilacją:**  
   Po wprowadzeniu poprawek, a przed kompilacją, sugerujesz treść wpisu do pliku `Kincony_inverters_delta_me300_VPCM0701s.md` opisującego wprowadzone zmiany (co zmieniono, gdzie, dlaczego i jak testować).